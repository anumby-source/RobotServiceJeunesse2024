...
# model.load_state_dict(torch.load(f'images-{config.N}-{config.epochs}.pth'))
# model.eval()
...
transform = v2.Compose([
    ...
    v2.Resize((config.image_size, config.image_size)),          # Redimensionner l'image à la taille d'entrée du modèle Inception (299x299)
    v2.Compose([v2.ToImage(), v2.ToDtype(torch.float32, scale=True)]),
    # transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])  # Normaliser l'image
    # transforms.Normalize(mean=[0.485], std=[0.229])  # Normaliser l'image (si nécessaire)
])
